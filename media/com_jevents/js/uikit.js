/*! gslUIkit 3.16.19 | https://www.getuikit.com | (c) 2014 - 2023 YOOtheme | MIT License */

(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
    typeof define === 'function' && define.amd ? define('uikit', factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.gslUIkit = factory());
        ...
        toNodes(element).map(
          (element2) => new Promise((resolve, reject) => {
            ...
            once(
              ...
              ({ type }) => {
                clearTimeout(timer);
                removeClass(element2, "gsl-transition");
                css(element2, {
                  transitionProperty: "",
                  transitionDuration: "",
            ...
            addClass(element2, "gsl-transition");
            css(element2, {
    ...
    const animationPrefix = "gsl-animation-";
        ...
        toNodes(element).map(
          (element2) => new Promise((resolve, reject) => {
            ...
            if (startsWith(animation, animationPrefix)) {
              origin && addClass(element2, `gsl-transform-origin-${origin}`);
    ...
    function globalApi(App) {
      ...
      App.extend = function(options) {
        options || (options = {});
        const Super = this;
        const Sub = function gslUIkitComponent(options2) {
    ...
    var Togglable = {
      ...
      data: {
        ...
        origin: false,
        transition: "ease",
        clsEnter: "gsl-togglabe-enter",
        clsLeave: "gsl-togglabe-leave"
      },
      computed: {
    ...
    var Position = {
      ...
      methods: {
        ...
        getPositionOffset(element) {
          return toPx(
            this.offset === false ? css(element, "--gsl-position-offset") : this.offset,
        ...
        getShiftOffset(element) {
          return this.align === "center" ? 0 : toPx(
            css(element, "--gsl-position-shift-offset"),
            this.axis === "y" ? "width" : "height",
            element
          ) * (includes(["left", "top"], this.align) ? 1 : -1);
        },
        getViewportOffset(element) {
          return toPx(css(element, "--gsl-position-viewport-offset"));
        }
      }
    };
    ...
    var drop = {
      ...
      beforeConnect() {
        this.clsDrop = this.$props.clsDrop || `gsl-${this.$options.name}`;
      },
      connected() {
        addClass(this.$el, "gsl-drop", this.clsDrop);
      ...
      methods: {
        ...
        position() {
          removeClass(this.$el, "gsl-drop-stack");
          css(this.$el, this._style);
          this.$el.hidden = true;
          ...
          if (this.$el.offsetWidth > maxWidth) {
            addClass(this.$el, "gsl-drop-stack");
          }
    ...
    var Dropnav = {
      ...
      data: {
        ...
        clsDrop: "gsl-dropdown",
        clsDropbar: "gsl-dropnav-dropbar",
        boundary: true,
        dropbar: false,
        dropbarAnchor: false,
        duration: 200,
    ...
    const NavParentIcon = {
      ...
      beforeConnect() {
        ...
        this.icon = closest(this.$el, ".gsl-nav-primary") ? `${icon}-large` : icon;
      }
    };
    const Search = {
      extends: IconComponent,
      ...
      beforeConnect() {
        this.icon = hasClass(this.$el, "gsl-search-icon") && parents(this.$el, ".gsl-search-large").length ? "search-large" : parents(this.$el, ".gsl-search-navbar").length ? "search-navbar" : this.$props.icon;
        if (hasAttr(this.$el, "aria-label")) {
          return;
        }
        if (hasClass(this.$el, "gsl-search-toggle") || hasClass(this.$el, "gsl-navbar-toggle")) {
    ...
    const Slidenav = {
      ...
      beforeConnect() {
        ...
        const icon = this.$props.icon;
        this.icon = hasClass(this.$el, "gsl-slidenav-large") ? `${icon}-large` : icon;
      }
    };
    ...
    const Close = {
      ...
      i18n: { label: "Close" },
      beforeConnect() {
        this.icon = `close-${hasClass(this.$el, "gsl-close-large") ? "large" : "icon"}`;
      }
    };
    const Marker = {
      extends: ButtonComponent,
    ...
    function install$3(gslUIkit) {
      gslUIkit.icon.add = (name, svg) => {
        ...
        if (gslUIkit._initialized) {
          apply(
            document.body,
            (el) => each(gslUIkit.getComponents(el), (cmp) => {
    ...
    function toMedia(value, element) {
      if (isString(value)) {
        if (startsWith(value, "@")) {
          value = toFloat(css(element, `--gsl-breakpoint-${value.substr(1)}`));
        } else if (isNaN(value)) {
          return value;
        }
      }
    ...
    var leader = {
      ...
      computed: {
        fill({ fill }) {
          return fill || css(this.$el, "--gsl-leader-fill-content");
        }
      },
      connected() {
    ...
    function install$2({ modal }) {
      modal.dialog = function(content, options) {
        const dialog = modal(
          `<div class="gsl-modal"> <div class="gsl-modal-dialog">${content}</div> </div>`,
      ...
      modal.alert = function(message, options) {
        return openDialog(
          ({ i18n }) => `<div class="gsl-modal-body">${isString(message) ? message : html(message)}</div> <div class="gsl-modal-footer gsl-text-right"> <button class="gsl-button gsl-button-primary gsl-modal-close" autofocus>${i18n.ok}</button> </div>`,
          options
        );
      };
      modal.confirm = function(message, options) {
        return openDialog(
          ({ i18n }) => `<form> <div class="gsl-modal-body">${isString(message) ? message : html(message)}</div> <div class="gsl-modal-footer gsl-text-right"> <button class="gsl-button gsl-button-default gsl-modal-close" type="button">${i18n.cancel}</button> <button class="gsl-button gsl-button-primary" autofocus>${i18n.ok}</button> </div> </form>`,
          options,
          () => Promise.reject()
        );
      };
      modal.prompt = function(message, value, options) {
        const promise = openDialog(
          ({ i18n }) => `<form class="gsl-form-stacked"> <div class="gsl-modal-body"> <label>${isString(message) ? message : html(message)}</label> <input class="gsl-input" value="${value || ""}" autofocus> </div> <div class="gsl-modal-footer gsl-text-right"> <button class="gsl-button gsl-button-default gsl-modal-close" type="button">${i18n.cancel}</button> <button class="gsl-button gsl-button-primary">${i18n.ok}</button> </div> </form>`,
          options,
          () => null,
          () => input.value
        );
    ...
    var navbar = {
      ...
      data: {
        ...
        selNavItem: ".gsl-navbar-nav > li > a,a.gsl-navbar-item,button.gsl-navbar-item,.gsl-navbar-item a,.gsl-navbar-item button,.gsl-navbar-toggle"
        // Simplify with :where() selector once browser target is Safari 14+
      },
      watch: {
        items() {
          const justify = hasClass(this.$el, "gsl-navbar-justify");
          for (const container of $$(
            ".gsl-navbar-nav, .gsl-navbar-left, .gsl-navbar-right",
            this.$el
          )) {
            css(
              container,
              "flexGrow",
              justify ? $$(
                ".gsl-navbar-nav > li > a, .gsl-navbar-item, .gsl-navbar-toggle",
                container
              ).length : ""
            );
          }
    ...
    var scrollspy = {
      ...
      observe: intersection({
        ...
        handler(records) {
          ...
          for (const { target: el, isIntersecting } of records) {
            if (!elements.has(el)) {
              elements.set(el, {
                cls: data(el, "gsl-scrollspy-class") || this.cls
              });
            }
      ...
      methods: {
        toggle(el, inview) {
          ...
          if (/\bgsl-animation-/.test(state.cls)) {
            const removeAnimationClasses = () => removeClasses(el, "gsl-animation-[\\w-]+");
            if (inview) {
    ...
    function applyAttributes(el) {
      const { $el } = this;
      addClass(el, attr($el, "class"), "gsl-svg");
    ...
    function applyAnimation(el) {
      ...
      if (length) {
        css(el, "--gsl-animation-stroke", length);
      }
    }

    const selDisabled = ".gsl-disabled *, .gsl-disabled, [disabled]";
    var Switcher = {
      mixins: [Togglable],
      args: "connect",
      props: {
    ...
    var tab = {
      ...
      data: {
        ...
        selVertical: ".gsl-tab-left,.gsl-tab-right"
      },
      connected() {
        const cls = hasClass(this.$el, "gsl-tab-left") ? "gsl-tab-left" : hasClass(this.$el, "gsl-tab-right") ? "gsl-tab-right" : false;
        if (cls) {
    ...
    const clsLeave = "gsl-transition-leave";
    const clsEnter = "gsl-transition-enter";
    ...
    function install$1(gslUIkit, Lightbox) {
    ...
    var notification = {
      ...
      created() {
        ...
        this.$mount(
          append(
            container,
            `<div class="${this.clsMsg}${this.status ? ` ${this.clsMsg}-${this.status}` : ""}" role="alert"> <a href class="${this.clsClose}" data-gsl-close></a> <div>${this.message}</div> </div>`
          )
        );
      },
      async connected() {
    ...
    function install(gslUIkit) {
      gslUIkit.notification.closeAll = function(group, immediate) {
        apply(document.body, (el) => {
          const notification = gslUIkit.getComponent(el, "notification");
    ...
    var tooltip = {
      ...
      methods: {
        ...
        async _show() {
          this.tooltip = append(
            this.container,
            `<div id="${this.id}" class="gsl-${this.$options.name}" role="tooltip"> <div class="gsl-${this.$options.name}-inner">${this.title}</div> </div>`
          );
