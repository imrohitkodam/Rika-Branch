window.onload = function(){
	...
	if (jQuery('#jevents #jevents_body').length)
	{
		jQuery(document).on('click', '#jevents #jevents_body .subform-repeatable-wrapper joomla-field-subform[name=tickettypes] fieldset.gsl-button-group label.gsl-button', function () {
			var forData = jQuery(this).attr('for');
			if (/sequence0$/.test(forData)) 
			{
				jQuery('label[for="' + forData + '"]').addClass('gsl-button-primary');
				// Replace the last character with '1'
				forData = forData.slice(0, -1) + "1";
				jQuery('label[for="' + forData + '"]').removeClass('gsl-button-danger');
			} else {
				jQuery('label[for="' + forData + '"]').addClass('gsl-button-danger');
				// Replace the last character with '1'
				forData = forData.slice(0, -1) + "0";
				jQuery('label[for="' + forData + '"]').removeClass('gsl-button-primary');
			}
		});
	}
}
...
var valid = {
	...
	startNumberValidation : function(es) {
		const alphanumericRegex = /^[a-zA-Z0-9]+$/;
		if (es.value)
		{
			if (!alphanumericRegex.test(es.value)) {
				jQuery(es).val('');
				alert(Joomla.JText._('COM_JTICKETING_TICKET_START_NUMBER_ERROR'));
