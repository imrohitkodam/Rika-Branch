...
var jtSite = {
	...
	order: {
		...
		gatewayHtml: function(element, orderId) {
			...
			if (typeof intervalCounterDown != 'undefined')
			{
				clearInterval(intervalCounterDown);
			}
			jQuery('.countdownDateInfo').html('');
			jQuery.ajax({
				beforeSend: function() {
				...
				success: function(result) {
					if (result.success === true) {
						...
						countdownDate.setMinutes(countdownDate.getMinutes() + (Number(intervalCounterDownTime))); //Add 5 minutes to it

						intervalCounterDown = setInterval(function() {
							...
							var output = Joomla.JText._('COM_JTICKETING_COMPLETE_PAYMENT_WITHIN') + " <b>" + minutes + "m " + seconds + "s </b>";
							jQuery('.countdownDateInfo').html(output);
								
							// If the count down is over, write some text 
							if (distance < 0) {
								clearInterval(intervalCounterDown);
								jQuery('.countdownDateInfo').html('Expired');

								alert(Joomla.JText._('COM_JTICKETING_TIME_EXPIRED_RETRY_PAYMENT'));
		...
		initEventJs: function() {
			...
			Joomla.submitbutton = function(task) {
				...
				if (task == "eventform.save") {
					...
					// check entry number configuration is enabled, if yes then applied respective validation
					if (jQuery('#jform_start_number_for_event_level_sequence').length)
					{
						const alphanumericRegex = /^[a-zA-Z0-9]+$/;
						// check start number of sequence in event level input only number and alphabetical present
						if (jQuery('#jform_start_number_for_event_level_sequence').val())
						{
							if (!alphanumericRegex.test(jQuery('#jform_start_number_for_event_level_sequence').val())) {
								var error_html = '';
								jQuery('#eventform-save').removeClass('disabled');
								error_html += Joomla.JText._('COM_JTICKETING_INVALID_FIELD') + ': ' + Joomla.JText._('COM_JTICKETING_START_NUMBER_FOR_EVENT_LEVEL_SEQUENCE');
								Joomla.renderMessages({"warning":[error_html]});
								jQuery('#jform_start_number_for_event_level_sequence').focus();
						...
						while(1)
						{
							var allowSequence = 'jform[tickettypes][tickettypes' + i + '][allow_ticket_level_sequence]';

							if (!jQuery('input[name="' + allowSequence +'"]:checked').length)
							{
								break;
							}

							if (jQuery('input[name="' + allowSequence +'"]:checked').val())
							{
								var sequenceId = '#jform_tickettypes__tickettypes' + i + '__start_number_for_sequence';

								if (jQuery(sequenceId).length && jQuery(sequenceId).val() && !alphanumericRegex.test(jQuery(sequenceId).val()))
								{
									var error_html = '';
									jQuery('#eventform-save').removeClass('disabled');
									error_html += Joomla.JText._('COM_JTICKETING_INVALID_FIELD') + ': ' + Joomla.JText._('COM_JTICKETING_START_NUMBER_FOR_SEQUENCE');
									Joomla.renderMessages({"warning":[error_html]});
									jQuery(sequenceId).focus();
