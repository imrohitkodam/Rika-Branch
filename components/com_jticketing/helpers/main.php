...
require JPATH_SITE . '/libraries/lib_tjbarcode/vendor/autoload.php';
...
class Jticketingmainhelper
{
	...
	public function getticketHTML($data, $usesession = 0)
	{
		...
		// Addied by 4CR to add Order ID for Ticket PDF
		if (isset($data->ord_id))
		{
			$msg['order_id'] = htmlspecialchars($data->ord_id, ENT_COMPAT, 'UTF-8');
		}
		...
		$msg['entry_number'] = htmlspecialchars($data->entry_number, ENT_COMPAT, 'UTF-8');
	...
	public function tagreplace($msg, $flag = ' ')
	{
		...
		$message_body = str_replace("[ORDER_ID]", $msg['order_id'], $message_body); // Added by 4CR to add Order ID for Ticket PDF
	...
	public function getticketDetails($eventid = '', $ticketid = '')
	{
		...
		$qrstring                 = urlencode($qrstring); // Added by 4CR to fix QR code for ticket email
		// $qr_url                = "https://quickchart.io/qr?chs";
		$qr_url                   = "https://quickchart.io/qr?chs="; // Edited by 4CR to fix QR code for ticket email
		// $qrimage               = $qr_url . $qr_code_width . "x" . $qr_code_height . "&chl=" . $qrstring . "&chld=H|0";
		$qrimage                  = $qr_url . $qr_code_width . "x" . $qr_code_height . "&text=" . $qrstring . "&chld=H|0"; // Edited by 4CR to fix QR code for ticket email
		...
		// Added by 4CR: START
		// Add QR code image tag for Ticket email template
		$ch = curl_init($qrimage);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$contents = curl_exec($ch);
		curl_close($ch); // Close the connection to save memory
		$result->qr_code = '<img src="data:image/png;base64,' . base64_encode($contents) . '" class="qrimg">';
		// Display ticket_type_price in Ticket email.
		$utilities = JT::utilities();
		$result->ticket_type_price = $utilities->getFormattedPrice($result->ticketprice);
		// Added by 4CR: END
