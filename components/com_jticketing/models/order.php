...
class JticketingModelOrder extends AdminModel
{
	...
	public function onAfterOrderComplete($order)
	{
		...
		// Update the attendee status.
		if (!$this->decrementPendingSeatsCount($order))
		{
			$this->setError($order->getError());
	 ...
	 * @since  2.5.0
	 */
	public function decrementTicketCount($order)
	{
		if (!empty($order))
		{
			...
			foreach ($orderDetails as $orderdetail)
			{
				...
				$query->where($db->quoteName('id') . " = " . (int) $orderdetail->type_id);

				$db->setQuery($query);
				$ticketTypeFromDatabase = $db->loadObject();

				if (!$ticketType->unlimited_seats)
				{
					if ($ticketTypeFromDatabase->count > 0)
					{
						$ticketType->count 	= max(0, $ticketType->count - $orderdetail->ticketcount);
						$ticketType->pending_seats 	= min($ticketType->available, $ticketType->pending_seats + $orderdetail->ticketcount);
	...
	public function getOrderStatues($type = 'fullforms')
	{
		...
		$statues = array(COM_JTICKETING_CONSTANT_ORDER_STATUS_PENDING,
					...
					COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER
				);
		...
		$fullForms = array(Text::_('JT_PSTATUS_PENDING'),
						...
						Text::_('COM_JTICKEITNG_PSTATUS_INFORMATION_GATHER')
					);
	...
	/**
	 * Function to update mdate.
	 *
	 * @param   JTicketingOrder  $order  Object of order
	 *
	 * @return  boolean  true when update columns successfully.
	 *
	 * @since  5.0.4
	 */
	public function updateMdate($order)
	{
		$ordData = array();
		$ordData['name'] = $order->name;
		$ordData['id'] = $order->id;
		$ordData['order_id'] = $order->order_id;
		$ordData['event_details_id'] = $order->event_details_id;
		$ordData['mdate'] = Factory::getDate()->toSql();
	 ...
	 * @since  2.5.0
	 */
	public function decrementPendingSeatsCount($order)
	{
		if (!empty($order))
		{
			$orderDetails 	= $order->getItems();
			...
			foreach ($orderDetails as $orderdetail)
			{
				...
				if (!$ticketType->unlimited_seats)
				{
					$ticketType->pending_seats 	= max(0, $ticketType->pending_seats - $orderdetail->ticketcount);
