...
class JticketingModelallticketsales extends BaseDatabaseModel
{
	...
	public function _buildQuery()
	{
		...
		$filter_order     = $app->getUserStateFromRequest($option . 'filter_order', 'filter_order', 'startdate', 'cmd');
		...
		if ($filter_order)
		{
			...
			{
				$query->order($db->escape($filter_order) . ' ' . $db->escape($filter_order_Dir));
			}
			...
			if ($table)
			{
				...
				if (in_array($filter_order, $allowed_fields) && in_array($filter_order_Dir, array('asc', 'desc')))
				{
					switch ($filter_order)
					{
						...
						case 'summary':
							$query->order($db->quoteName($filter_order) . ' ' . $filter_order_Dir);
							break;
	...
	public function _buildContentXrefid()
	{
		...
		$eventid = $mainframe->input->get('event', '', 'INT');
