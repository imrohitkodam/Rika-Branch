...
class JticketingModelAttendees extends ListModel
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$status = $app->getUserStateFromRequest($this->context . 'filter.status', 'filter_status', 'A', 'string');
		...
		$attendeeListingFields = $params->get('attendee_listing_fields', ['COM_JTICKETING_ENROLMENT_USER_USERNAME','COM_JTICKEITNG_ATTENDEE_EMAIL','COM_JTICKETING_ENROLMENT_ACTION','COM_JTICKETING_CHECKIN'], 'ARRAY');
		if (in_array('COM_JTICKEITNG_ATTENDEE_ENTRY_NUMBER', $attendeeListingFields))
		{
			parent::populateState('oitem.entry_number', 'asc');
		}
		else
		{
		...
		if (!empty($search))
		{
			...
			if ($integration == 3)
			{
				$query->where('('.
					...
					. ' OR ' . $db->quoteName('type.title') . ' LIKE ' . $search . ' OR '
			...
			else
			{
				$conditions = $db->quoteName('events.title') . ' LIKE ' . $search . ' OR ' .
								...
								$db->quoteName('type.title') . ' LIKE ' . $search . ' OR ' .
