...
class JticketingModelpayment extends BaseDatabaseModel
{
	...
	public function getHTML($order_id, $gateway = '')
	{
		if (!empty($order_id) && is_int($order_id))
		{
			...
			if (in_array($order->getStatus(), [COM_JTICKETING_CONSTANT_ORDER_STATUS_INCOMPLETE, COM_JTICKETING_CONSTANT_ORDER_STATUS_FAILED, COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER]))
			{
				$bindData           = array();
	...
	public function updateOrderGateway($selectedGateway, $order_id)
	{
		...
		$row->mdate     = Factory::getDate()->toSql();
	...
	public function getReceiverList($vars, $pg_plugin, $orderid)
	{
		...
		if ($pluginParams && property_exists($pluginParams, 'business'))
		{
