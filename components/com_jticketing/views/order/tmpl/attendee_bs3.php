...
<form name="attendee_field_form" action="" id="attendee_field_form" class="form-validate attendee_form container-fluid af-pl-0 af-pr-0">
	<div class="row af-mt-10">
		<div class="col-sm-8">
		 <?php echo $this->loadTemplate('event_info_' . JTICKETING_LOAD_BOOTSTRAP_VERSION);?>
			...
			<div>
				...
				{?>
				<div class="custom-new-panel mt-20 panel panel-default br-5 m-0">
					<div class="panel-heading br-t5">
						<h3 class="panel-title"><strong><?php echo Text::_('COM_JTICKETING_SEND_TICKETS');?></strong>
						</h3>
					</div>
		...
		foreach ($this->orderItems AS $key => $oitem)
		{
			...
			{ ?>
				<div>
					<div class="custom-new-panel af-mt-20 panel panel-default af-br-5 af-m-0">
						<div class="panel-heading af-br-t5">
							<h3 class="panel-title"><strong><?php echo Text::sprintf('COM_JTICKETING_ATTENDEE_INFORMATION', $tickettype->title);?></strong></h3>
						</div>
						<div class="panel-body">
...
-->
							...
							foreach ($oitem->attendee_value AS $field)
							{
								...
								<div class="form-group">
									<div class="input-field col-sm-6 col-xs-12 af-mb-20">
										<?php
										if(!empty($field->type))
										{
