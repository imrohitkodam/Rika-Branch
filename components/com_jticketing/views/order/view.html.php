...
class JticketingViewOrder extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
		...
		if (in_array($this->order->getStatus(), [COM_JTICKETING_CONSTANT_ORDER_STATUS_PENDING, COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER]))
		{
			$userId = Factory::getUser()->id;
			if ($this->event->isOver() || $this->event->isBuyingLimitExceed($userId))
			{
				$app->enqueueMessage(Text::_('COM_JTICKETING_ORDER_NOT_ACCESSIBLE'), 'message');
				$app->redirect($this->event->getUrl());
			}
		}
		else if (!$this->event->isAllowedToBuy())
		{
			$app->enqueueMessage(Text::_('COM_JTICKETING_ORDER_NOT_ACCESSIBLE'), 'message');
			$app->redirect($this->event->getUrl());
		}
		...
		if (!empty($this->orderItems))
		{
			...
			{
				foreach ($this->ticketTypeData as $ticketTypeCheck)
				{
					if ($ticketTypeCheck->id == $orderData->type_id 
						&& $ticketTypeCheck->unlimited_seats == 0
						&& $ticketTypeCheck->count > 0
						// Seems to be we have added this for check attendee-edit, but we need this function in future so commented for now
						// && $ticketTypeCheck->count == $this->event->getTicketCountByOrder($this->orderItems, $orderData->type_id) 
					)
					{
		...
		if ($layout == 'payment')
		{
			if (!$this->order->reValidateAmount() && ($this->order->getStatus() != 'P' || $this->order->getStatus() != 'IG'))
			{
			...
			// Re-secure the tickets.
			if ($this->order->getStatus() === COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER || $this->order->getStatus() === COM_JTICKETING_CONSTANT_ORDER_STATUS_PENDING)
			{
				$orderModel->updateMdate($this->order);
			}
			else if (!empty($orderItems))
			{
				foreach ($orderItems as $orderData)
				{
					if (!$this->event->isSeatsAvailable($this->order, $orderData->type_id))
					{
						$app->enqueueMessage(Text::_('COM_JTICKETING_SEATS_NOT_AVAILABLE'), 'message');
						$app->redirect($this->event->getUrl());
					}
				}

				$result = $ordersModel->changeOrderStatus($this->order, COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER);
				if (!$result)
				{
					$app->enqueueMessage($ordersModel->getError(), 'message');
					$app->redirect($this->event->getUrl());
				}
				
				$orderModel->decrementTicketCount($this->order);
				$orderModel->updateMdate($this->order);
			}
			...
			$this->intervalCounterDownTime = $this->jtParams->get("order_exceed_time", "5");
		}
