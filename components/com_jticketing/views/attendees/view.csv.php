...
class JticketingViewAttendees extends TjExportCsv
{
	...
	public function display($tpl = null)
	{
		...
		if ($userAuthorisedExport !== true || !$user->id)
		{
			...
			$app->redirect($redirect, Text::_('JERROR_ALERTNOAUTHOR'));
		...
		else
		{
			...
			else
			{
				...
				foreach ($this->items as $key => $item)
				{
					...
					{
						$this->data[$key]->email = $this->escape($item->attendee_email);
					}
					else
					{
						$this->data[$key]->email = $this->escape($item->owner_email);
					}

					$this->data[$key]->eventTitle  = $item->title;
					$this->data[$key]->ticketTypeTitle = $item->ticket_type_title;
