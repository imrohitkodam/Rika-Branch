	...
	protected function addTJtoolbar()
	{
		...
		if (!empty($this->items))
		{
			if ($canDo->get('core.edit.state'))
			{
				...
				$message['btn-name']   = Text::_("COM_JTICKETING_CSVEXPORT");
			...
			if (JVERSION >= '4.0.0')
			{
				$printText = Text::_('COM_JTICKETING_PRINT');

				$toolbar->appendButton(
					'Custom',
					'<a class="af-d-block af-relative btn btn-small btn-primary print-attendee-list ms-2" 
					href="javascript:void(0);"><span class="icon-print icon-white"></span> ' . $printText . '</a>'
				);
			}
		}
