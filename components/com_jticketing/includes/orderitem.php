...
class JTicketingOrderItem extends CMSObject
{
	...
	public function reValid($eventId)
	{
		...
		// The call is from order reValidateAmount while changing payment tab If the order status is already chekced then no need to check seat availability
		$checkSeatAvailability = false;

		if (($ticketType->price != $this->ticket_price) || !$ticketType->isValidToBuy($eventId, $checkSeatAvailability))
		{
			return false;
		}
