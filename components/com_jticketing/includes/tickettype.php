...
class JTicketingTickettype extends CMSObject
{
	 ...
	 * @param   bool  $checkSeatAvailability  check the seat availability
	 *
	 * @return  integer|Boolean  for unlimited seats false and for limited the count will be returned.
	 *
	 * @since   2.5.0
	 */
	public function isValidToBuy($eventId = null, $checkSeatAvailability = true)
	{
		...
		if ((!$this->id) || ($this->eventid != $eventId)
			|| ($this->state != 1) || ($checkSeatAvailability && !$this->isAvailable()) || $this->isExpired() || !in_array($this->access, $userAccessLevel))
		{
