...
class JTicketingOrder extends CMSObject
{
	...
	public function bind($array)
	{
		...
		if (isset($array['processor']))
		{
			$this->processor = $array['processor'];
		}
	...
	public function loadOrCreate()
	{
		...
		if ($this->isNew() && !empty($this->user_id))
		{
			...
			if (!$event->isSeatsAvailableForBooking())
			{
				$this->setError(Text::_('COM_JTICKETING_SEATS_NOT_AVAILABLE'));
	...
	private function calculateFee()
	{
		...
		{
			if ($siteAdminFeeCap > 0 && $siteAdminFeeCap && $feeAmount > $siteAdminFeeCap)
			{
		...
		$siteAdminFeePer = isset($vendorSpecificFee->percent_commission)?$vendorSpecificFee->percent_commission:$params->get('siteadmin_comm_per');
		$siteAdminFlatFee = isset($vendorSpecificFee->flat_commission) ? $vendorSpecificFee->flat_commission :$params->get('siteadmin_comm_flat');
	...
	public function reValidateAmount()
	{
		...
		foreach ($ticketStat as $ticketId => $stat)
		{
			...
			if (!($this->getStatus() === COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER || $this->getStatus() === COM_JTICKETING_CONSTANT_ORDER_STATUS_PENDING))
			{
	...
	public function delete()
	{
		...
		// Check whether order is locked any seat then unlock it
		if (in_array($this->getStatus(), [COM_JTICKETING_CONSTANT_ORDER_STATUS_PENDING, COM_JTICKETING_CONSTANT_ORDER_STATUS_INFORMATION_GATHER]))
		{
			...
			foreach ($itemTypes as $item)
			{
				...
				if (empty($item->unlimited_seats))
				{
					...
					$item->pending_seats 	= max(0, $item->pending_seats - $itemStat->count);
					$item->count = (int) min($item->available, $item->count + $itemStat->count);
					$item->save();
				}
			}
		}
	...
	/**
	 * Method to reverse order and set attendee status to rejected
	 *
	 * @return  void
	 *
	 * @since   3.3.0
