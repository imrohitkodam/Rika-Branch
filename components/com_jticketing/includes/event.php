...
class JTicketingEvent extends CMSObject
{
	...
	/**
	 * Get ticket availabe count
	 *
	 * @return  bool  ticket available 
	 *
	 * @since   4.2.1
	 */
	public function isSeatsAvailable($order, $typeId)
	{
		$db = Factory::getDbo();
		$itemTypes = $order->getItemTypes();
		...
		foreach ($itemTypes as $item)
		{
			...
			if (!($item->unlimited_seats))
			{
				$query = $db->getQuery(true);
				$query->select('sum(oitem.ticketcount)')
					->where($db->quoteName('o.status') . ' = ' . $db->quote('P'))
					->where($db->quoteName('o.event_details_id') . ' = ' . (int) $order->event_details_id)
					->where($db->quoteName('o.id') . ' != ' . (int) $order->id)
					->where($db->quoteName('oitem.type_id') . ' = ' . (int) $typeId)
					->from($db->quoteName('#__jticketing_order', 'o'))
					->join('LEFT', $db->qn('#__jticketing_order_items', 'oitem') . 'ON (' . $db->qn('oitem.order_id')
							. ' = ' . $db->qn('o.id') . ')');

				$db->setQuery($query);
				$pendingStateOrdersCount = $db->loadResult();
				...
				$db->setQuery($query);
				$currentOrdersCount = $db->loadResult();
				$currentOrdersCount = $currentOrdersCount ? $currentOrdersCount : 0;

				$ticketCount += (int) $pendingStateOrdersCount + (int) $currentOrdersCount;

				if ((int)$item->count != 0 && ((int)$currentOrdersCount <= (int)$item->count)) {
					return true;
				}
	...
	/**
	 * Determine if the event is avialable to book ticket
	 *
	 * @return  boolean return true if the event is over
	 *
	 * @since   2.5.0
	 */
	public function isSeatsAvailableForBooking()
	{
		$ticketTypes = $this->getTicketTypes();
