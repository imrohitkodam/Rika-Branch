...
class JticketingControllerOrder extends JticketingController
{
	...
	public function addOrder()
	{
		...
		if ((!$allowGuest && !$user->id) || ($integration == 'com_jticketing' && $event->online_events && !$user->id))
		{
			...
			$isJEventIntegration = $config->get('integration', 0);
		
			if($isJEventIntegration == 3){
				$eventUrl               = 'index.php?option=com_jevents&task=icalrepeat.detail&evid=' . $eventID . '&Itemid=' . $itemId;
			}
			else{
