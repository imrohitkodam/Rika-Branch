 ...
 * @since  1.6
 */
class JticketingControllerVenues extends AdminController
{
	/**
	 * Proxy for getModel.
	 *
	...
	public function publish()
	{
		...
		else
		{
			...
			{
				echo $buttonVal = "-2";
				$this->setMessage(Text::_('COM_JTICKETING_VENUE_TRASHED'));
			}
			else
			{
				echo $buttonVal = "";
		...
		$this->setRedirect(Route::_('index.php?option=com_jticketing&view=venues&Itemid=' . $itemId, false));
	}
	...
	public function delete()
	{
		...
		{
			$msg = Text::sprintf(Text::_('COM_JTICKETING_VENUE_ALREADY_IN_USE'), count($venueDetails));
			...
			{
				$model->delete($cid);
				$msg = Text::sprintf(Text::_('COM_JTICKETING_VENUES_DELETED'), count($cid));
				$app->enqueueMessage($msg);
			}
