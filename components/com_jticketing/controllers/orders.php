...
class JticketingControllerorders extends BaseController
{
	 ...
	 * @deprecated 3.2.0 Use JT::model('orders')->addPendingEntriesToQueue(); instead
	 */
	public function cancelPendingOrdersThatTimeExceed()
	{
		$db = Factory::getDbo();
		...
		$Jticketingmainhelper   = new Jticketingmainhelper;
		$com_params             = ComponentHelper::getParams('com_jticketing');
		$cancelOrderKey         = $com_params->get("cancel_order_key");
		$orderExceedTime         = $com_params->get("order_exceed_time");
		$input                  = Factory::getApplication()->input;
		$privateKeyinurl       = $input->get('cancel_order_key', '', 'STRING');
		$offlinePaymentGateway = ['bycheck', 'byorder'];
		...
		if ($cancelOrderKey != $privateKeyinurl)
		{
			echo Text::_('COM_JTICKETING_NOT_AUTHORIED_FOR_DECLINE_ORDER_STATUS_ERROR');;
		...
		$db    = Factory::getDbo();
		$fields = array(
			$db->quoteName('o.status') . ' = ' . $db->quote('E'),
		);
		$conditions = array( 
			$db->quoteName('o.status') . ' = ' . $db->quote('P'),
			$db->quoteName('o.processor') . ' NOT IN (' . implode(',', $db->Quote($offlinePaymentGateway)) . ')',
			$db->quoteName('o.mdate') .  '< DATE_SUB(UTC_TIMESTAMP(),INTERVAL '. $orderExceedTime .' MINUTE)'
		);
		...
		$query->select($db->quoteName('id'))
			->where('((' . 
				...
				$db->quoteName('o.processor') . ' NOT IN (' . implode(',', $db->quote($offlinePaymentGateway)) . '))' . 
				' OR ' . 
				$db->quoteName('o.status') . ' = ' . $db->quote('IG') .
			')')
			->where($db->quoteName('o.mdate') .  '< DATE_SUB(UTC_TIMESTAMP(),INTERVAL '. $orderExceedTime .' MINUTE)')
			->from($db->quoteName('#__jticketing_order', 'o'));

		$db->setQuery($query);

		$orders= $db->loadColumn();

		JLoader::import('orders', JPATH_SITE . '/components/com_jticketing/models');

		try
		{
			foreach ($orders as $key => $order_id) 
			{
				Factory::getApplication()->input->post->set('payment_status', COM_JTICKETING_CONSTANT_ORDER_STATUS_FAILED);

				// Increase the ticket type count.
				$order        	= JT::order($order_id);

				/** @var $ordersModel JticketingModelorders */
				$ordersModel 	= JT::model('orders');
				$result 		= $ordersModel->changeOrderStatus($order, COM_JTICKETING_CONSTANT_ORDER_STATUS_FAILED);
			}

			echo Text::_('COM_JTICKETING_CHANGE_ORDER_STATUS_FAILED_SUCCESS_MESSAGE');;
		}
		catch (RuntimeException $e)
		{
			echo Text::_('COM_JTICKETING_CHANGE_ORDER_STATUS_FAILED_ERROR_MESSAGE');;
