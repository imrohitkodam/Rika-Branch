...
class JFormFieldCron extends JFormField
{
	...
	protected function getInput()
	{
		switch ($this->name)
		{
			...
			case 'jform[cancel_order_cron_url]':
	...
	protected function getCronUrl($name, $value, $node, $control_name)
	{
		...
		switch ($name)
		{
			...
			case 'jform[cancel_order_cron_url]':
				$private_key_cronjob = $params->get('cancel_order_key');

				$cron_masspayment = Route::_(
								Uri::root() . 'index.php?option=com_jticketing&task=orders.cancelPendingOrdersThatTimeExceed&cancel_order_key='
								. $private_key_cronjob
							);
				$return	= '<label class="text-break">' . $cron_masspayment . '</label>';
