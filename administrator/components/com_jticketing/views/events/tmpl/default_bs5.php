					...
					foreach ($this->items as $i => $item) :
						...
						<td class="">
							...
							<?php if ($canEdit) : ?>
								...
								<br>
								<?php echo Text::_("COM_JTICKETING_EVENTS_CATEGORY") . ": " . $item->catid;?>
							<?php else : ?>
...
-->
						...
						<a onclick="document.getElementById('previewModal<?php echo $item->id;?>').open();"
						...
						<td>
							<div class="btn-group">
								...
								echo HTMLHelper::_(
								'bootstrap.renderModal', 'attendeeexport' . $item->id,
								array(
								...
								?>
								<a class="btn"
								onclick="document.getElementById('attendeeexport<?php echo $item->id; ?>').open();"
								...
								if($this->params->get('signin_export'))
								{
									...
									echo HTMLHelper::_(
									'bootstrap.renderModal', 'signinsheet' . $item->id,
									array(
								...
								?>
								<a class="btn"
								onclick="document.getElementById('signinsheet<?php echo $item->id; ?>').open();"
								...
								if($this->params->get('namecard_export'))
								{
									...
									echo HTMLHelper::_(
								  	'bootstrap.renderModal', 'namecard' . $item->id,
								  	array(
								...
								?>
								<a class="btn"
								onclick="document.getElementById('namecard<?php echo $item->id; ?>').open();"
						...
						<td class="center">
							<button type="submit" class="tbody-icon btn btn-micro active hasTooltip"
							onclick="return Joomla.listItemTask('cb<?php echo $i;?>','<?php echo ($item->featured) ? 'events.unfeature' : 'events.feature';?>')"
							title="<?php echo ( $item->featured ) ? Text::_('COM_JTICKETING_UNFEATURE_ITEM') : Text::_('COM_JTICKETING_FEATURE_ITEM'); ;?>">
								<?php
									$featuredClass = ($item->featured) ? 'color-featured icon-star' : 'unfeatured';
								?>
								<span class="icon-<?php echo $featuredClass;?>"></span>
							</button>
						</td>
...
$body = '<div id="import_events" class="container">
